		...
		if($swissoc =~ /Mammalia/ && $frag == 0 && $ftnumber == 1 && $U==0 ){
			if($swisssu =~ /type II |typeII /){
				...
				for($i=0;$i<@su;$i++){
					...
					if($note==0 && $su[$i] =~ /Nuclear|nuclear|Nucleus|nucleus/){
				...
				if($t2eco!=255 && $pmeco==269 && $goleco==0 && $ereco==0 && $nueco==0 && $mito==0 && $lysosome==0 && $melanosome==0 && $lipidanchor==0){
